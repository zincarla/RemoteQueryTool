...
namespace Remote_Query_Tool
{
    public partial class RQT : Form
    {
        ...
        /*
        /// <summary>
        /// Performs a program/service query against one computer in the list
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        void BWProgram_DoWork(object sender, DoWorkEventArgs e)
        {
            #region Ping Setup
            //Sets up the ping codes
            Ping pingSender = new Ping();
            ...
            while (stopWorkers == false)
            {
                if (FinishedReads == true && ComputerBuffer.Count == 0)
                {
                    break;
                }
                ...
                try
                {
                    if (Computer != "N/A" && Computer != null)
                    {
                        currentComputerText(Computer, Index);
                        ...
                        if (reply != null && reply.Status == IPStatus.Success && Err == "")
                        {
                            ...
                            try
                            {
                                ...
                                if (ServicesRB.Checked == true)//Service query
                                {
                                    ...
                                    if (SingleRB.Checked == true)//Query against 1 service
                                    {
                                        ...
                                        while (pos < Services.Length)//Search through all services
                                        {
                                            if (Services[pos].DisplayName == toSearchFor.Text)//for the service to find
                                            {
                                                found = true;
                                                string Status = Services[pos].Status.ToString();
                                                if (restartCheckBox.Checked && Services[pos].CanStop)//If restart requested
                                                {
                                                    ...
                                                    try
                                                    {
                                                        ...
                                                        try
                                                        {
                                                            Services[pos].WaitForStatus(ServiceControllerStatus.Stopped, new TimeSpan(0, 5, 0));
                                                            try
                                                            {
                                                                Status += "/Start";
                                                                Services[pos].Start();
                                                    ...
                                                    catch (Exception ex) { Status += "/Fail"; }
                                                }
                                                toWrite[1] = toSearchFor.Text;
                                                toWrite[2] = Status;
                                                break;
                                            }
                                        ...
                                        {
                                            toWrite[1] = toSearchFor.Text;
                                            toWrite[2] = "Not installed";
                                        }
                                    }
                                    else if (MultipleRB.Checked == true)//Query to list services
                                    {
                                        toWrite[1] = "Service Name";
                                        toWrite[2] = "Status";//First row for column headers
                                        AddToBuffer(toWrite);//Write
                                        int pos = 0;
                                        while (pos < Services.Length)
                                        {
                                            toWrite[1] = Services[pos].DisplayName;//Any other row is informational following header layout
                                            toWrite[2] = Services[pos].Status.ToString();
                                            AddToBuffer(toWrite);//Write
                                            pos++;
                                        }
                                        toWrite = null;//Prevents the write request at bottom of this method
                                    }
                                }
                                else if (ProcessRB.Checked == true)//Query process
                                {
                                    ...
                                    {
                                        Process[] Processes = Process.GetProcesses(Computer);
                                        toWrite[1] = "Process Name";
                                        toWrite[2] = "";
                                        AddToBuffer(toWrite);//Write colum row (computer name and column headers
                                        int pos = 0;
                                        while (pos != Processes.Length)
                                        {
                                            toWrite[1] = Processes[pos].ProcessName;
                                            AddToBuffer(toWrite);//Write a row per process
                                            pos++;
                                        }
                                        toWrite = null;//Prevent the write at bottom of this method
                                    }
                            ...
                            {
                                toWrite[1] = "ERROR";
                                toWrite[2] = err.Message;//Error handle
                                //Errors += toWrite + "\r\n";
                            }
                        ...
                        {
                            toWrite[1] = "Computer Offline";//Ping failed (computer offline)
                            toWrite[2] = "Computer Offline";
                        }
                            #endregion
                        if (toWrite != null)
                        {
                            AddToBuffer(toWrite);//Occurs if not a list or an error occured
                        }
                        IncrementProgress();
                    }
                    else
        ...
        /// <summary>
        /// Performs a registry query against one computer in the list
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        void BWRegistry_DoWork(object sender, DoWorkEventArgs e)
        {
            #region Ping Setup
            //Sets up the ping codes
            Ping pingSender = new Ping();
            ...
            while (stopWorkers == false)
            {
                ...
                try
                {
                    if (Computer != "N/A" && Computer != null)
                    {
                        ...
                        if (reply != null)
                        {
                            if (reply.Status == IPStatus.Success && Err == "")
                            {
                                ...
                                try
                                {
                                    ...
                                    RegistryKey RK = RegistryKey.OpenRemoteBaseKey(KeyHive, Computer.Trim());
                                    value = RK.OpenSubKey(KeyPath, RegistryKeyPermissionCheck.ReadSubTree, System.Security.AccessControl.RegistryRights.ReadKey).GetValue(KeyName).ToString();
                                    RK.Close();
                                    if (existsRB.Checked == true)//check only for existance
                                    {
                                        //toWrite += FullPath + "\tTrue";
                                        toWrite[1] = FullKeyPath;
                                        toWrite[2] = "True";
                                    }
                                    else if (valueRB.Checked == true)//Value check
                                    {
                                    ...
                                    {
                                        //toWrite += FullPath + "\t" + value;
                                        toWrite[1] = FullKeyPath;
                                        toWrite[2] = value;
                                    }
                                }
                                #region Exception catcher and error handling
                                catch (ArgumentException)
                                {
                                    ...
                                    {
                                        //toWrite += FullPath + "\t" + "Key Not found";
                                        toWrite[1] = FullKeyPath;
                                        toWrite[2] = "Key Not Found";
                                    }
                                }
                                catch (NullReferenceException)
                                {
                                ...
                                {
                                    //toWrite += "ERROR\t" + err.Message.Replace("\r\n", "").Replace("\t", "");
                                    //Errors += toWrite + "\r\n";
                                    toWrite[1] = "ERROR";
                                    toWrite[2] = err.Message;
                                }

                            }
                            else
                            {
                                //toWrite += "\tComputer Offline\t" + "Computer Offline";
                                toWrite[1] = "Computer Offline";
                                toWrite[2] = Err;
                            }
                        }
                        else
        ...
        //TODO For any new type of query, create a new DoWork method
        ///*void NAME_DoWork(object sender, DoWorkEventArgs e)
        //{
        //    int Index = ClaimIndex();
            
        //    //TODO Initialize
        //    while (stopWorker == false && !(FinishReads == true && Computers.Count == 0))
        //    {
        //        if (FinishReads == true && Computers.Count == 0)
        //        {
        //            break;
        //        }
        //        string Computer = RetrieveFromComputerBuffer();
        ...
        //                //currentComputerText(Computer, Index);
        //                //Do query
        //                //Set any statuses with currentComputerText("MyStatus", Index);
        //                //Write the outcome with AddToBuffer(toWrite);
        //                IncrementProgress();
        //            }
        //            else
        //            {
        ...
        /// <summary>
        /// Performs commands per each computer, remoting is done by user per command (%machine% set to help)
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        void BWCommands_DoWork(object sender, DoWorkEventArgs e)
        {
            #region Ping Setup
            //Sets up the ping codes
            Ping pingSender = new Ping();
            ...
            while (stopWorkers == false)
            {
                ...
                try
                {
                    if (Computer != "N/A" && Computer != null)
                    {
                        ...
                        {
                            AddToBuffer(new string[3] { Computer, "Computer Offline", "Computer Offline" });
                        }
                        IncrementProgress();
                    }
                    else
        ...
        */
        ...
        public void SaveOptions(string Path)
        {
            ...
            /*try
            {
                ...
                {
                    SW.WriteLine("[RQT Options File]");
                    if (singleQuery.Checked == true)
                    {
                        SW.WriteLine("[Input Type {(l)ist, (s)ingle}]s");
                    }
                    else if (multipleQueries.Checked == true)
                    {
                        SW.WriteLine("[Computer Type {(l)ist, (s)ingle}]l");
                    }
                    SW.WriteLine("[PC Name]" + pcName.Text);
                    SW.WriteLine("[List Path]" + inputTB.Text);
                    SW.WriteLine("[Output Path]" + outputTB.Text);
                    SW.WriteLine("[Simultaneous Queries]" + simultaneousQ.Value.ToString());
                    if (currentUserRB.Checked)
                    {
                        SW.WriteLine("[Hive Key {(cu)Current User, (lm)Local Machine, (u)Users}]cu");
                    }
                    else if (localMachineRB.Checked)
                    {
                        SW.WriteLine("[Hive Key {(cu)Current User, (lm)Local Machine, (u)Users}]lm");
                    }
                    else if (usersRB.Checked)
                    {
                        SW.WriteLine("[Hive Key {(cu)Current User, (lm)Local Machine, (u)Users}]u");
                    }
                    SW.WriteLine("[Key Path]" + keyPathTB.Text);
                    if (existsRB.Checked)
                    {
                        SW.WriteLine("[Reqistry Query Type {(e)xists, (v)alue equals, (o)utput values}]e");
                    }
                    else if (valueRB.Checked)
                    {
                        SW.WriteLine("[Reqistry Query Type {(e)xists, (v)alue equals, (o)utput values}]v");
                    }
                    else if (outputRB.Checked)
                    {
                        SW.WriteLine("[Reqistry Query Type {(e)xists, (v)alue equals, (o)utput values}]o");
                    }

                    SW.WriteLine("[Key Equals]" + valueTB.Text);

                    if (ProcessRB.Checked)
                    {
                        SW.WriteLine("[Process/Service Query {(p)rocess, (s)ervice}]p");
                    }
                    else if (ServicesRB.Checked)
                    {
                        SW.WriteLine("[Process/Service Query {(p)rocess, (s)ervice}]s");
                    }

                    if (SingleRB.Checked)
                    {
                        SW.WriteLine("[P/S Query Type {(f)ind, (l)ist}]f");
                    }
                    else if (MultipleRB.Checked)
                    {
                        SW.WriteLine("[P/S Query Type {(f)ind, (l)ist}]l");
                    }

                    SW.WriteLine("[P/S To Find]" + toSearchFor.Text);
                    SW.WriteLine("[P/S Restart]" + restartCheckBox.Checked.ToString());
                }
            }
            catch (Exception EXC)
            {
                MessageBox.Show("Save Failed\r\n" + EXC.Message, "Save Failed");
            }*/
            #endregion
        }
        ...
        public void LoadOptions(string Path)
        {
            ...
            /*try
            {
                if (File.Exists(Path))
                {
                    ...
                    {
                        FullOptions = SR.ReadToEnd();
                    }

                    string[] Options = FullOptions.Replace("\r\n", "").Split(new string[2] { "[", "]" }, StringSplitOptions.None);
                    int pos = 0;
                    while (pos < Options.Length)
                    {
                        string Line = Options[pos];

                        if (Line == "Input Type {(l)ist, (s)ingle}")
                        {
                            pos++;
                            if (Options[pos].Trim().ToLower() == "s")
                            {
                                singleQuery.Checked = true;
                            }
                            else if (Options[pos].Trim().ToLower() == "l")
                            {
                                multipleQueries.Checked = true;
                            }
                        }
                        else if (Line == "PC Name")
                        {
                            pos++;
                            pcName.Text = Options[pos];
                        }
                        else if (Line == "List Path")
                        {
                            pos++;
                            inputTB.Text = Options[pos];
                        }
                        else if (Line == "Output Path")
                        {
                            pos++;
                            outputTB.Text = Options[pos];
                        }
                        else if (Line == "Simultaneous Queries")
                        {
                            pos++;
                            simultaneousQ.Value = Convert.ToInt64(Options[pos]);
                        }
                        else if (Line == "Hive Key {(cu)Current User, (lm)Local Machine, (u)Users}")
                        {
                            pos++;
                            if (Options[pos].Trim().ToLower() == "cu")
                            {
                                currentUserRB.Checked = true;
                            }
                            else if (Options[pos].Trim().ToLower() == "lm")
                            {
                                localMachineRB.Checked = true;
                            }
                            else if (Options[pos].Trim().ToLower() == "u")
                            {
                                usersRB.Checked = true;
                            }
                        }
                        else if (Line == "Key Path")
                        {
                            pos++;
                            keyPathTB.Text = Options[pos];
                        }
                        else if (Line == "Reqistry Query Type {(e)xists, (v)alue equals, (o)utput values}")
                        {
                            pos++;
                            if (Options[pos].Trim().ToLower() == "e")
                            {
                                existsRB.Checked = true;
                            }
                            else if (Options[pos].Trim().ToLower() == "v")
                            {
                                valueRB.Checked = true;
                            }
                            else if (Options[pos].Trim().ToLower() == "o")
                            {
                                outputRB.Checked = true;
                            }
                        }
                        else if (Line == "Key Equals")
                        {
                            pos++;
                            valueTB.Text = Options[pos];
                        }
                        else if (Line == "Process/Service Query {(p)rocess, (s)ervice}")
                        {
                            pos++;
                            if (Options[pos].Trim().ToLower() == "p")
                            {
                                ProcessRB.Checked = true;
                            }
                            else if (Options[pos].Trim().ToLower() == "s")
                            {
                                ServicesRB.Checked = true;
                            }

                        }
                        else if (Line == "P/S Query Type {(f)ind, (l)ist}")
                        {
                            pos++;
                            if (Options[pos].Trim().ToLower() == "f")
                            {
                                SingleRB.Checked = true;
                            }
                            else if (Options[pos].Trim().ToLower() == "l")
                            {
                                MultipleRB.Checked = true;
                            }

                        }
                        else if (Line == "P/S To Find")
                        {
                            pos++;
                            toSearchFor.Text = Options[pos];
                        }
                        else if (Line == "P/S Restart")
                        {
                            pos++;
                            bool CheckedValue;
                            if (bool.TryParse(Options[pos].Trim().ToLower(), out CheckedValue))
                            {
                                restartCheckBox.Checked = CheckedValue;
                            }
                        }
                ...
                {
                    throw new FileNotFoundException("The options file was not found");
                }
            }
            catch (Exception EXC)
            {
